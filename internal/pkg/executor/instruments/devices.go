...
func NewDeviceLister(find executor.ExecutableFinder, execute executor.Executor) DeviceLister {
	return func() ([]DeviceEntry, error) {
		if err := find("instruments"); err != nil {
			return nil, err
		}
		result, execErr := execute(executor.NewRequest("instruments", []string{"-s", "devices"}))
		if execErr != nil {
			return nil, execErr
		}
