...
func NewInteractiveExecutor(find executor.ExecutableFinder, execute executor.InteractiveExecutor) InteractiveExecutor {
	return func(ctx context.Context, stdin io.ReadCloser, stdout io.Writer, stderr io.Writer, args ...string) error {
		...
		req := executor.NewInteractiveRequest(stdin, stdout, stderr, "adb", args)
